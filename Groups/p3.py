...
# Output folder
pout = os.getcwd() + '/Groups/Analysis/'
...
pin = os.getcwd() + '/Groups/Outputs/'# + visit# + '_v2'

# Loading the spectra...
lc_all, lc_err_all = [], []
times = np.array([])
for i in range(len(segs)):
    dataset1 = pickle.load(open(pin + '/Spectrum_cube_seg' + segs[i] + '_' + visit + '.pkl', 'rb'))
...
fname = open(pin + '/White_light.dat', 'w')
for i in range(len(tim7)):
    fname.write(str(tim7[i]) + '\t' + str(fl7[i]) + '\t' + str(fle7[i]) + '\n')
fname.close()

plt.errorbar(tim[visit], fl[visit], yerr=fle[visit], fmt='.')
plt.plot(tim[visit], fl[visit], 'k-', alpha=0.5, zorder=5)
plt.show()
...
par_P = ['P_p1', 't0_p1', 'p_p1_' + visit, 'b_p1', 'q1_' + visit, 'q2_' + visit, 'ecc_p1', 'omega_p1', 'a_p1']
